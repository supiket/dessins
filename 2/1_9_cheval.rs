    ...
    for i in -n..=n {
        for j in -n..=n {
            ...
            loop {
                ...
                let x_ = (a + j as f32 * 20.0 - 20.0) * np / 80.0;
                let y_ = (b + i as f32 * 20.0 - 20.0) * np / 80.0;

                let di = (x_ * x_ + y_ * y_).sqrt();

                let an = if x_ != 0.0 {
                    (y_ / x_).atan() + f32::PI() * (1.0 - if x_ < 0.0 { -1.0 } else { 1.0 }) / 2.0
                } else {
                    f32::PI() / 2.0 * if y_ < 0.0 { -1.0 } else { 1.0 }
                };
                let di = di / np * 3.0;
                let di = di / (1.0 + di) * np * 0.65;
