        ...
        for j in 0..2 {
            ...
            loop {
                ...
                if a == 1000 {
                    ...
                    points_index += 1;
                    polyline_index += 1;
                    points[i as usize * 2 + j].push(vec![]);
                }
                let a = a as i32;
                let b = settings.points[points_index] as i32;
                points_index += 1;

                let x = (1.0 - 2.0 * j as f32) * settings.np as f32 * a as f32 / 80.0 * r;
                let y = settings.np as f32 * (0.5 - r + b as f32 / 80.0 * r);
                points[i as usize * 2 + j][polyline_index as usize].push(pt2(x, y));
            }
        }
    }
}
