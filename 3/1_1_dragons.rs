...
fn calculate_dragon_points(model: &mut Model) {
    model.settings.initial_angle = -f32::PI() / 4.0 * (model.settings.n - 2) as f32;
    model.settings.l0 = model.settings.np as f32 / (2.0_f32.sqrt().powf(model.settings.n as f32));
    model.rules = vec![0; model.settings.n + 1];
...
fn update(_app: &App, model: &mut Model, update: Update) {
...

    {
        ...
        egui::Window::new("settings").show(&ctx, |ui| {
            ...
            ui.label("n:");
            if ui
                .add(egui::Slider::new(&mut model.settings.n, 2..=14))
                .changed()
            {
                recalculate = true;
            }
...
fn view(app: &App, model: &Model, frame: Frame) {
    ...
    if model.points.len() > 1 {
        for i in 0..model.points.len() - 1 {
            let start = model.points[i] * 0.8;
            let end = model.points[i + 1] * 0.8;
